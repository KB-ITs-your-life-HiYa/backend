...
`db/seed/R__*.sql` 의 기능별 파일에 넣고 커밋한다.
예: `R__seed_01_member.sql`, `R__seed_02_housing.sql`.
로컬·테스트 프로필에서 적용하며, 테스트는 별도 `fledge_test` DB를 사용한다.
...
상담사 개발용 계정은 `counselor@fledge.local` / `counselor1234`이며,
로그인 후 프론트의 상담사 포털로 분기한다. 포털 관리 데이터는 현재 프론트 모의 데이터다.
