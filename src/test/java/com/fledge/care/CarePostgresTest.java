...
class CarePostgresTest {
    ...
    @Test void migrationsPersistenceResetAndSevenDayRecheck() {
        ...
        assertThat(care.setDemoDate(2L, LocalDate.of(2026, 9, 24)).riskScore()).isEqualTo(35);
        long utility = care.summary(2L).signals().getFirst().id();
        var reply = care.respond(2L, utility, new ButtonRequest(Choice.DIFFICULT, "pg-policy", null, null))
        ...
        care.savePolicies(2L, utility, reply.id(), new Policies("READY", List.of(card)));
        assertThat(care.summary(2L).signals().getFirst().replies().getFirst().policies().cards()).containsExactly(card);
        assertThat(care.setDemoDate(2L, LocalDate.of(2026, 9, 26)).riskScore()).isEqualTo(75);
        ...
        assertThat(jdbc.queryForObject("select count(*) from referral_request where member_id=2", Integer.class)).isZero();
        assertThat(jdbc.queryForObject("select count(*) from transaction where member_id=2", Integer.class)).isEqualTo(82);
        care.setDemoDate(2L, LocalDate.of(2026, 9, 24));
        jdbc.update("insert into transaction(member_id,account_id,txn_date,txn_type,amount,merchant_name) values(2,11,'2026-09-24','EXPENSE',38900,'한국전력·도시가스')");
        care.setDemoDate(2L, LocalDate.of(2026, 10, 1));
